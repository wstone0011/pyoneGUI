...
#----LibPlugin----
class MyPyInstArchive(PyInstArchive):
    def extractFiles(self):
        print('[+] Beginning extraction...please standby')
...
class LibPluginPyinstxtractor():
    @staticmethod
    def extractFiles(file):
        cwd_path=os.getcwd()
        arch = MyPyInstArchive(file)
        result=""
        if arch.open():
            if arch.checkFile():
                if arch.getCArchiveInfo():
                    ...
                    os.chdir(cwd_path) #arch.extractFiles()会调用os.chdir(extractionDir)会更改路径，这里回到原路径
                    
                    result+='[+] Successfully extracted pyinstaller archive: {0}\n'.format(file)
                    result+='\n'
                    result+='You can now use a python decompiler on the pyc files within the extracted directory\n'
                    return result
