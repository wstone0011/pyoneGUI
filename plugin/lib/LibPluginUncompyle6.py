...
class LibPluginUncompyle6():
    ...
    def Decompile(file):
        if file.endswith(".pyc"):
            ...
            outfile=file+".py"
        print(outfile)
        plugin_argv=["uncompyle6", "-o", outfile, file]
        LibPluginUncompyle6.main_bin(plugin_argv)
        
    def main_bin(plugin_argv):
        #基于python-uncompyle6/uncompyle6/bin/uncompile.py
        recurse_dirs = False
        numproc = 0
        outfile = "-"
        out_base = None
        ...
        try:
            ...
            print("%s: %s" % (os.path.basename(plugin_argv[0]), e), file=sys.stderr)
            #sys.exit(-1)
